...
class TestSidecar(unittest.TestCase):
    ...
    def test_get_ready(self):
        r = safe_requests.get(f"{self.server_url }/ready")
    ...
    def test_get_substitutions(self):
        ...
        r = safe_requests.get(f"{self.server_url }/substitutions")
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json(), expected)

    def test_get_check_hash_no_found(self):
        r = safe_requests.get(f"{self.server_url }/check_hash/foobar")
    ...
    def test_get_check_hash_nested(self):
        ...
        with open(test_path, "w") as f:
            f.write("hello world")
        r = safe_requests.get(f"{self.server_url }/check_hash/nested/foobar")
        self.assertEqual(r.status_code, 200)
        self.assertEqual(
            r.text, "b94d27b9934d3e08a52e52d7da7dabfac484efe37a5380ee9088f7ace2efcde9"
        )
    ...
    def test_get_is_present_no_found(self):
        r = safe_requests.get(f"{self.server_url }/is_present/foobar")
        self.assertEqual(r.status_code, 404)
        self.assertRegex(r.text, "foobar not found")
    ...
    def test_get_is_present_nested(self):
        ...
        with open(test_path, "w") as f:
            f.write("hello world")
        r = safe_requests.get(f"{self.server_url }/is_present/nested/foobar")
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.text, "OK\n")

    def test_get_not_found(self):
        r = safe_requests.get(f"{self.server_url }/foobar")
        self.assertEqual(r.status_code, 404)
